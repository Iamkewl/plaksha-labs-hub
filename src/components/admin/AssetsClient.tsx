...
type AssetStatus = "All" | "available" | "in-use" | "checked-out" | "maintenance" | "retired";

const KIND_OPTIONS: Kind[] = ["All", "Machine", "Tool", "Equipment"];
const STATUS_OPTIONS: AssetStatus[] = ["All", "available", "in-use", "checked-out", "maintenance", "retired"];
...
export function AssetsClient({ assets }: AssetsClientProps) {
  const labOptions: string[] = ["All", ...Array.from(new Set(assets.map((a) => a.lab))).sort()];
